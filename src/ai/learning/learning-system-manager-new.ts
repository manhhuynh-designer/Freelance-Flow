...
import { BehaviorTracker, type ActionEvent, type UserBehaviorPattern } from './behavior-tracker';
import { PatternAnalyzer } from './pattern-analyzer';
import { AdaptivePredictor } from './adaptive-predictor';
...
export class LearningSystemManager {
  ...
  constructor(private userId: string) {
    ...
    this.adaptivePredictor = new AdaptivePredictor(userId, [], [], {});
  ...
  async trackAction(action: ActionEvent): Promise<void> {
    ...
    try {
      ...
      await this.behaviorTracker.trackAction(action);
  ...
  async generatePersonalInsights(
    ...
    try {
      ...
      // Get behavior data
      const actions = await this.behaviorTracker.getActions();
      const patterns = await this.patternAnalyzer.analyzePatterns(actions);
      ...
      insights.push(...this.extractPatternInsights(patterns));
  ...
  async generatePredictions(
    ...
    try {
      ...
      return await this.adaptivePredictor.predict(
        actions,
        patterns,
        { tasks, ...context }
      );
  ...
  async processFeedback(
    ...
    try {
      await this.adaptivePredictor.updateModel({
        predictionId,
        feedback,
        details,
        timestamp: new Date()
  ...
  async getSystemState(): Promise<LearningSystemState> {
    try {
      const actions = await this.behaviorTracker.getActions();
      const patterns = await this.patternAnalyzer.analyzePatterns(actions);
      const predictions = await this.adaptivePredictor.getPredictions();
      const accuracy = this.adaptivePredictor.getAccuracy();
      ...
      return {
        ...
        lastAnalysis: actions.length > 0 ? new Date() : null,
