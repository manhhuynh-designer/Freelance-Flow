...
export function EditTaskForm({ setOpen, onSubmit: onFormSubmit, taskToEdit, quote, collaboratorQuote, clients, collaborators, categories, onAddClient, quoteTemplates, settings }: EditTaskFormProps) {
    ...
    const [isCollaboratorSectionOpen, setIsCollaboratorSectionOpen] = useState(isEditMode && !!collaboratorQuote && (collaboratorQuote.sections || []).length > 0);
        ...
        defaultValues: useMemo(() => {
            const getInitialSections = (): QuoteSection[] => {
                if (quote?.sections && quote.sections.length > 0) {
                    ...
                    items: (section.items && section.items.length > 0)
                        ? section.items.map((item, itemIndex) => ({ ...item, id: item.id || `item-edit-${sectionIndex}-${itemIndex}` }))
                        : [{ id: `item-edit-${sectionIndex}-0`, description: "", quantity: 1, unitPrice: 0, customFields: {} }]
                    }));
                }
                return [{
                    id: `section-edit-${taskToEdit.id}`,
                    name: T.untitledSection,
                    items: [{ id: `item-edit-${taskToEdit.id}-0`, description: "", quantity: 1, unitPrice: 0, customFields: {} }]
                }];
            };
            ...
            const getInitialCollabSections = (): QuoteSection[] => {
                if (collaboratorQuote?.sections && collaboratorQuote.sections.length > 0) {
                    ...
                    items: (section.items || []).map((item, itemIndex) => ({ ...item, id: item.id || `item-collab-edit-${sectionIndex}-${itemIndex}` }))
                    }));
                }
                return [];
            };
            ...
            return {
                ...
                briefLink: taskToEdit.briefLink || "",
                ...
                dates: { from: new Date(taskToEdit.startDate), to: new Date(taskToEdit.deadline) },
                sections: getInitialSections(),
                collaboratorSections: getInitialCollabSections(),
            }
        }, [taskToEdit, quote, collaboratorQuote, T.untitledSection])
    });

    const [columns, setColumns] = useState<QuoteColumn[]>(
      isEditMode && quote?.columns ? quote.columns : defaultColumns
    );
    const [collaboratorColumns, setCollaboratorColumns] = useState<QuoteColumn[]>(
      isEditMode && collaboratorQuote?.columns ? collaboratorQuote.columns : defaultColumns
    );
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-4">
                  <div className="flex gap-4">
                    <FormField control={form.control} name="status" render={({ field }) => (<FormItem className="flex-1"><FormLabel>{T.status}</FormLabel><Select onValueChange={(value) => { field.onChange(value); form.setValue('subStatusId', ''); }} value={field.value}><FormControl><SelectTrigger><SelectValue placeholder={T.selectStatus} /></SelectTrigger></FormControl><SelectContent>{(settings.statusSettings || []).map((status) => (<SelectItem key={status.id} value={status.id}>{status.label}</SelectItem>))}</SelectContent></Select><FormMessage /></FormItem>)} />
                    {availableSubStatuses.length > 0 && (<FormField control={form.control} name="subStatusId" render={({ field }) => (<FormItem className="flex-1"><FormLabel>{T.subStatuses}</FormLabel><Select onValueChange={(value) => field.onChange(value === '__none__' ? '' : value)} value={field.value || '__none__'}><FormControl><SelectTrigger><SelectValue placeholder={T.selectSubStatus} /></SelectTrigger></FormControl><SelectContent><SelectItem value="__none__">{T.none}</SelectItem>{availableSubStatuses.map((sub) => (<SelectItem key={sub.id} value={sub.id}>{sub.label}</SelectItem>))}</SelectContent></Select><FormMessage /></FormItem>)} />)}
                  </div>
                </div>
              </div>
            </div>
            ...
            <Collapsible open={isCollaboratorSectionOpen} onOpenChange={setIsCollaboratorSectionOpen}><div className="space-y-4"><div className="flex justify-between items-center"><CollapsibleTrigger asChild><button type="button" className="flex items-center w-full group"><h3 className="text-lg font-medium">{T.collaboratorCosts}</h3><ChevronDown className="h-4 w-4 ml-2 transition-transform duration-200 group-data-[state=open]:rotate-180" /></button></CollapsibleTrigger><Button type="button" variant="outline" size="sm" onClick={handleCopyFromQuote}><Copy className="mr-2 h-4 w-4" />{T.copyFromPriceQuote}</Button></div><CollapsibleContent className="space-y-4 data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down">
              <FormField control={form.control} name="collaboratorId" render={({ field }) => (<FormItem><FormLabel>{T.collaborator}</FormLabel><Select onValueChange={(value) => field.onChange(value === '__none__' ? '' : value)} value={field.value || '__none__'}><FormControl><SelectTrigger><SelectValue placeholder={T.selectCollaborator} /></SelectTrigger></FormControl><SelectContent><SelectItem value="__none__">{T.none}</SelectItem>{(collaborators || []).map((collaborator) => (<SelectItem key={collaborator.id} value={collaborator.id}>{collaborator.name}</SelectItem>))}</SelectContent></Select><FormMessage /></FormItem>)} />
            ...
            <div className="flex justify-end gap-2"><Button type="button" variant="ghost" onClick={() => setOpen(false)}>{T.cancel}</Button><Button type="submit">{isEditMode ? T.saveChanges : T.createTask}</Button></div>
          </form>
        </Form>
