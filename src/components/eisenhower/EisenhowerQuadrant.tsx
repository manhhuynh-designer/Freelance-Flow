...
export function EisenhowerQuadrant({ id, title, description, tasks, onClearQuadrant }: EisenhowerQuadrantProps) {
  ...
  const colorScheme = dashboardContext?.appSettings?.eisenhowerColorScheme || 'colorScheme1';
  
  // Get color classes for each quadrant (from CSS module)
  ...
  return (
    <TooltipProvider>
      <div className={`${styles[colorScheme]}`}>
        <Card className={`flex flex-col h-full transition-all duration-200 relative ${getQuadrantColors(id)} ${isOver ? 'ring-2 ring-primary ring-offset-1' : ''}`}>
          ...
          <SortableContext items={tasks.map(task => task.id)} strategy={rectSortingStrategy}>
            <div className={`${styles.dropZone} relative h-full transition-all duration-200 rounded-lg z-10`}>
              {tasks.length === 0 ? (
                ...
                <div className={`grid grid-cols-1 sm:grid-cols-2 gap-2 overflow-y-auto h-full p-2 min-h-[120px] ${isOver ? 'bg-primary/5 rounded-lg border-2 border-dashed border-primary' : ''}`}>
                  {tasks.map(task => (
                  ...
                  <div className={`w-full h-16 ${isOver ? 'border-2 border-dashed border-primary/50 rounded-lg bg-primary/5' : ''}`} />
                </div>
              )}
            </div>
          </SortableContext>
